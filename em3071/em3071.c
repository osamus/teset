...
#if defined(MT6575)||defined(MT6572)||defined(MT6589)
#define POWER_NONE_MACRO MT65XX_POWER_NONE
#endif
...
/* zhaoshaopeng Add for auto detect feature */
...
static int em3071x_init_flag =0;
static struct sensor_init_info em3071x_init_info = {			
	.name = "em3071x",			
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	printk("em3071_ps_operate command =%d \n",command);
	//APS_FUN(f);
	switch (command)
	{
		case SENSOR_DELAY:
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				//em3071_read_als(obj->client, &obj->als);
				mutex_unlock(&sensor_lock);
				APS_ERR("em3071_ps_operate als ps data=%d !\n",obj->ps);
				sensor_data->values[0] = em3071_get_ps_value(obj, obj->ps);
				sensor_data->value_divide = 1;
				sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;			
			}
